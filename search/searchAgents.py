...
class CornersProblem(search.SearchProblem):
    ...
    cornersVisited = set()
    startState = position PacMan + cornersVisited
    goalState = all corners visited, i.e. len(cornersVisited) == 4
    """
    ...
    def getStartState(self):
        ...
        "*** YOUR CODE HERE ***"
        # state is encoded
        return (frozenset(), self.startingPosition)
    ...
    def isGoalState(self, state: Any):
        ...
        """
        "*** YOUR CODE HERE ***"
        return all([corner in state[0] for corner in self.corners])
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [
            ...
            if not hitsWall:
                newCornersVisited = (
                    cornersVisited.union({(nextx, nexty)})
                    if (nextx, nexty) in self.corners
                    else cornersVisited
                )
                nextState = (newCornersVisited, (nextx, nexty))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    what could be a good heuristic?
    i.e. distance to closest corner + sum of min distance to next unvisited corner
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    "*** YOUR CODE HERE ***"
    distances = [
        mazeDistance((j, i), position, problem.startingGameState)
        for j, i in foodGrid.asList()
    ]
