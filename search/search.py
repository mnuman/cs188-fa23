...
def aStarSearch(problem: SearchProblem, heuristic=nullHeuristic):
    ...
    while not frontier.isEmpty():
        ...
        if problem.isGoalState(node.state):
            ...
            visited.add(node.state)
            for new_state, new_action, new_cost in problem.getSuccessors(node.state):
                # heuristic is only taken into account for priority - not for cost!
                new_node = Node(
                    new_state, node.actions.copy() + [new_action], node.cost + new_cost
                )
                frontier.update(new_node, new_node.cost + heuristic(new_state, problem))
    return None
